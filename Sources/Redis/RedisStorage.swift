...
final class RedisStorage: Sendable {
        ...
        var pools: [PoolKey: RedisConnectionPool] {
            willSet {
                guard self.pools.isEmpty else {
                    fatalError(
                        "Modifying connection pools after application has booted is not supported"
                    )
                }
            }
        }
    ...
    func use(
        _ redisConfiguration: RedisConfiguration,
        as id: RedisID = .default
    ) {
        ...
        -> RedisConnectionPool
    {
        ...
        guard let pool = self.box.withLockedValue({ $0.pools[key] }) else {
            fatalError(
                "No redis found for id \(redisID), or the app may not have finished booting. Also, the eventLoop must be from Application's EventLoopGroup."
            )
        }
        return pool
    }
...
extension RedisStorage {
    ...
    final class Lifecycle: LifecycleHandler {
            ...
            for eventLoop in application.eventLoopGroup.makeIterator() {
                redisStorage.box.withLockedValue { storageBox in
                    for (redisID, configuration) in storageBox.configurations {
                        ...
                        let newKey: PoolKey = PoolKey(
                            eventLoopKey: eventLoop.key,
                            redisID: redisID
                        )

                        let redisTLSClient: ClientBootstrap? = {
                            guard
                                let tlsConfig = configuration.tlsConfiguration,
                                let tlsHost = configuration.tlsHostname
                            else { return nil }
                        ...
                        let newPool = RedisConnectionPool(
                            configuration: .init(
                                configuration,
                                defaultLogger: application.logger,
                                customClient: redisTLSClient
                            ),
        ...
        func shutdown(_ application: Application) {
            let shutdownFuture: EventLoopFuture<Void> = redisStorage.box
                .withLockedValue { $0.pools.values }.map { pool in
            ...
            do {
                ...
                application.logger.error(
                    "Error shutting down redis connection pools, possibly because the pool never connected to the Redis server: \(error)"
                )
            }
        }
...
extension RedisStorage {
    ...
    fileprivate struct PoolKey: Hashable, StorageKey {
...
extension EventLoop {
    fileprivate typealias Key = ObjectIdentifier
